...
const chatData = {
  ...
  function openChat(chatId) {
    const chat = chatData[chatId]; // Get the chat data based on the chatId
    const chatHeader = document.querySelector('.user-info h2');
    chatHeader.textContent = chat.name; // Update the chat header
  ...
  document.querySelectorAll('.chat-link').forEach(link => {
    link.addEventListener('click', e => {
      ...
      openChat(link.dataset.chat); // Call the function to open the chat
    });
  });